...
Socket.prototype = {
  ...
  onOpen: function() {
	...
	// console.log(this.vm.$auth.$storage.getCookie(`_token.${strategy}`))
	let temp = this.vm.$auth.$storage.getCookie(`_token.${strategy}`);
	let token = ''
	// console.log(temp)
	if(temp == undefined){
		token = ''
		...
		token = this.vm.$auth.$storage.getCookie(`_token.${strategy}`)
		  .split(" ")[1];
	}
