                ...
                <div class="swiper-wrapper">
                  <div
                    ...
                    <div v-for="(item,i) in baseinfo" :key="i">
                      ...
                      <!-- 指垫指套的显示及颜色控制 开始-->
                        ...
                        <div v-if="['web_style'].includes(item.placeN)&&showbaseindex==i&&tabIndex==0&&index==0">
                            <img :src="`/images/base/web/1/${getBaseValue('球挡样式')}.png`"  class="thumbnail" />
                        </div>
                        <!-- LOGO-->
                        <div style="position:relative;z-index:20;">
                      ...
                      <!-- 指垫-->
                      <div style="position:relative;top:70px;left:45px;" v-if="tabIndex==0&&getBaseValue('指垫')=='指垫'">
                          <div class="logo-img" :style="`background-image:url(/images/base/finger_pad/pad.png)`"/>
                      </div>
                      <!-- 手掌烫印 logo-->
                      <div style="position:relative;z-index:20;">
                      ...
                      <!-- 本垒国旗-->
                      <div style="position:relative;z-index:20;">
                          <div class="logo-img" :style="`background-image:url(/images/base/thumb_flag/${buyarr.productId}/${getBaseValue('拇指圈国旗')}-${index+1}.png)`" v-if="getBaseValue('拇指圈LOGO')=='国旗'" style="opacity:0.15;"/>
                      </div>
                      <!-- 指垫指套的显示及颜色控制 结束-->
                    </div>
                    ...
                    <div class="color-mask" v-for="item in colorarr.slice()" :key="item.placeN">
                      <span ref="colormask" :id="item.placeN+index" :class="'figure figure-'+item.placeN+'-'+(index+1)" :style="getColorStyle(item.title)" v-if="!(item.placeN=='PAD_HOOD'&&getBaseValue('指垫')=='无')"></span>
                    </div>
...
import cd from "../plugins/colordata_open_back_pro44.js"; //保存颜色列表
...
@import '@/assets/mask/open_back/iconfont.css';
  // 形状和文本叠与底图之上
  .color-mask{
    position: absolute;
    top: 0;
