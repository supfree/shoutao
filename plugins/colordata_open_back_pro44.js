module.exports = {
  "baseinfo": [
		{title:"尺寸",showtype:"radio",placeN:'size',activeI:'null',selectlist:[
      {showtxt:'12.5"',showvalue:'12.5"'},
      {showtxt:'12.75"',showvalue:'12.75"'},
      {showtxt:'13"',showvalue:'13"'},
      {showtxt:'13.25"',showvalue:'13.25"'},
      {showtxt:'13.5"',showvalue:'13.5"'},
      {showtxt:'13.75"',showvalue:'13.75"'},
      {showtxt:'14"',showvalue:'14"'},
    ]},
    {title:"LOGO类型",showtype:"radio",placeN:'logo_style',activeI:'null',selectlist:[
      {showtxt:'刺绣',showvalue:'刺绣'},
      {showtxt:'绣花章',showvalue:'绣花章'},
      {showtxt:'皮章',showvalue:'皮章'}
    ]},
    {title:"指垫",showtype:"radio",placeN:'finger_pad',activeI:'null',selectlist:[
      {showtxt:'无',showvalue:'无'},
      {showtxt:'指垫',showvalue:'指垫'},
    ]},
    {title:"编绳长度",showtype:"radio",placeN:'lace_length',activeI:'null',selectlist:[
      {showtxt:'正常',showvalue:'正常'},
      {showtxt:'长',showvalue:'长'},
    ]},
    {title:"手腕内里材质",showtype:"radio",placeN:'wrist_lining',activeI:'null',selectlist:[
      {showtxt:'传统羊毛垫',showvalue:'传统羊毛垫'},
      {showtxt:'吸汗垫',showvalue:'吸汗垫'},
    ]},
    {title:"球挡样式",showtype:"radio",placeN:'web_style',activeI:'null',selectlist:[
      {showtxt:'2-Piece Web',showvalue:'2-Piece Web'},
      {showtxt:'H-Web',showvalue:'H-Web'},
      {showtxt:'Post Web',showvalue:'Post Web'},
    ]},
    {title:"左右手",showtype:"radio",placeN:'throwing_hand',activeI:'null',selectlist:[
      {showtxt:'左',showvalue:'左'},
      {showtxt:'右',showvalue:'右'},
    ]},
    {title:"软硬度",showtype:"radio",placeN:'glove_stiffness',activeI:'null',selectlist:[
      {showtxt:'专业硬度',showvalue:'专业硬度'},
      {showtxt:'标准',showvalue:'标准'},
		]},




  ],
  "colordata": [
    {
      title: '皮革1', placeN: 'LEATHER_1', activeI: 'null', colorlist: [
      ]
    },
    {title:'手腕',placeN:'WRIST',activeI:'null',colorlist:[
    ]},
    {
      title: '手掌', placeN: 'PALM', activeI: 'null', colorlist: [
      ]
    },
    {title:'球挡',placeN:'WEB',activeI:'null',colorlist:[
    ]},
    {title:'指垫颜色',placeN:'PAD_HOOD',activeI:'null',colorlist:[
    ]},
    {
      title: '包边', placeN: 'BINDING', activeI: 'null', colorlist: [
      ]
    },
    {
      title: '编绳', placeN: 'LACES', activeI: 'xiedai', colorlist: [
      ]
    },
    {
      title: '缝线', placeN: 'STITCHING', activeI: 'null', colorlist: [
      ]
    },
    {title:'虎口',placeN:'WEB_CROWN',activeI:'null',colorlist:[
    ]},
  

    {
      title: 'LOGO颜色', placeN: 'LOGO_COLOR', activeI: 'null', colorlist: [
      ]
    },






 

    
  ],
  "colortxtdata": [
    {title:"拇指圈LOGO",showtype:"radio",placeN:'thumb_logo',activeI:'null',selectlist:[
      {showtxt:'无',showvalue:'无'},
      {showtxt:'数字',showvalue:'数字'},
      {showtxt:'国旗',showvalue:'国旗'},
    ]},
    {title:'数字',showtype:"text",placeN:'jumbo_number',style:[undefined,{top:90,left:250,size:80,transform:5}],activeI:'null',colorlist:[
      {colorV:'#5C4032',colorN:'CC28'},
      {colorV:'#772F09',colorN:'PA28'},
      {colorV:'#817F42',colorN:'PA42'},
      {colorV:'#D23745',colorN:'PT23'},
      {colorV:'#328A72',colorN:'PA47'},
      {colorV:'#874A97',colorN:'PT60'},
      {colorV:'#FEFEFE',colorN:'SY01'},
      {colorV:'#EBDFAF',colorN:'Sy03'},
      {colorV:'#A2A6A5',colorN:'Sy11'},
      {colorV:'#E7C84B',colorN:'Sy15'},
      {colorV:'#E3DA59',colorN:'Sy16'},
      {colorV:'#F47BA4',colorN:'SY19'},
      {colorV:'#F87936',colorN:'Sy22'},
      {colorV:'#F0C96A',colorN:'Sy27'},
      {colorV:'#5D8E78',colorN:'Sy40'},
      {colorV:'#91C771',colorN:'Sy43'},
      {colorV:'#2851BA',colorN:'Sy45'},
      {colorV:'#091023',colorN:'SY50'},
      {colorV:'#7BADC6',colorN:'Sy55'},
      {colorV:'#141416',colorN:'Sy90'},
      {colorV:'#FF8B00',colorN:'SYE27'},
      {colorV:'#D49F5D',colorN:'SyL28'},
      {colorV:'#827263',colorN:'SYQ28'},
    ]},
    {title:"拇指圈国旗",showtype:"radio",placeN:'stamped_flag',activeI:'null',selectlist:[
      {showtxt: '美国', showvalue: '美国'},  
      {showtxt: '亚利桑那州', showvalue: '亚利桑那州'},  
      {showtxt: '澳大利亚', showvalue: '澳大利亚'},  
      {showtxt: '加利福尼亚州', showvalue: '加利福尼亚州'},  
      {showtxt: '加拿大', showvalue: '加拿大'},  
      {showtxt: '芝加哥', showvalue: '芝加哥'},  
      {showtxt: '科罗拉多州', showvalue: '科罗拉多州'},  
      {showtxt: '古巴', showvalue: '古巴'},  
      {showtxt: '多米尼加共和国', showvalue: '多米尼加共和国'},  
      {showtxt: '日本', showvalue: '日本'},  
      {showtxt: '韩国', showvalue: '韩国'},  
      {showtxt: '马里兰州', showvalue: '马里兰州'},  
      {showtxt: '墨西哥', showvalue: '墨西哥'},  
      {showtxt: '波多黎各', showvalue: '波多黎各'},  
      {showtxt: '得克萨斯州', showvalue: '得克萨斯州'},  
      {showtxt: '委内瑞拉', showvalue: '委内瑞拉'},  
    ]},
    {title:"手掌烫印",showtype:"radio",placeN:'palm_sign',activeI:'null',selectlist:[
      {showtxt:'无',showvalue:'无'},
      {showtxt:'Aces logo',showvalue:'Aces logo'},
      {showtxt:'定制数字',showvalue:'定制数字'},
    ]},
    {title:'定制数字',showtype:"text",placeN:'palm_sign_text',style:[undefined,undefined,{top:160,left:440,size:80,transform:10}],activeI:'null',colorlist:[]},
		//自定义文本数组中的placeN的值，跟轮播图中的input的id名统一起来
		{title:'拇指文字',showtype:"text",placeN:'thumb_text',style:[undefined,{top:210,left:500,size:30,transform:5}],activeI:'null',colorlist:[
      {colorV:'#5C4032',colorN:'CC28'},
      {colorV:'#772F09',colorN:'PA28'},
      {colorV:'#817F42',colorN:'PA42'},
      {colorV:'#D23745',colorN:'PT23'},
      {colorV:'#328A72',colorN:'PA47'},
      {colorV:'#874A97',colorN:'PT60'},
      {colorV:'#FEFEFE',colorN:'SY01'},
      {colorV:'#EBDFAF',colorN:'Sy03'},
      {colorV:'#A2A6A5',colorN:'Sy11'},
      {colorV:'#E7C84B',colorN:'Sy15'},
      {colorV:'#E3DA59',colorN:'Sy16'},
      {colorV:'#F47BA4',colorN:'SY19'},
      {colorV:'#F87936',colorN:'Sy22'},
      {colorV:'#F0C96A',colorN:'Sy27'},
      {colorV:'#5D8E78',colorN:'Sy40'},
      {colorV:'#91C771',colorN:'Sy43'},
      {colorV:'#2851BA',colorN:'Sy45'},
      {colorV:'#091023',colorN:'SY50'},
      {colorV:'#7BADC6',colorN:'Sy55'},
      {colorV:'#141416',colorN:'Sy90'},
      {colorV:'#FF8B00',colorN:'SYE27'},
      {colorV:'#D49F5D',colorN:'SyL28'},
      {colorV:'#827263',colorN:'SYQ28'},
    ]},
    {title:"食指文字",showtype:"text",placeN:'index_text',style:[{top:-400,left:-220,size:30,transform:270}],activeI:'null',colorlist:[
      {colorV:'#5C4032',colorN:'CC28'},
      {colorV:'#772F09',colorN:'PA28'},
      {colorV:'#817F42',colorN:'PA42'},
      {colorV:'#D23745',colorN:'PT23'},
      {colorV:'#328A72',colorN:'PA47'},
      {colorV:'#874A97',colorN:'PT60'},
      {colorV:'#FEFEFE',colorN:'SY01'},
      {colorV:'#EBDFAF',colorN:'Sy03'},
      {colorV:'#A2A6A5',colorN:'Sy11'},
      {colorV:'#E7C84B',colorN:'Sy15'},
      {colorV:'#E3DA59',colorN:'Sy16'},
      {colorV:'#F47BA4',colorN:'SY19'},
      {colorV:'#F87936',colorN:'Sy22'},
      {colorV:'#F0C96A',colorN:'Sy27'},
      {colorV:'#5D8E78',colorN:'Sy40'},
      {colorV:'#91C771',colorN:'Sy43'},
      {colorV:'#2851BA',colorN:'Sy45'},
      {colorV:'#091023',colorN:'SY50'},
      {colorV:'#7BADC6',colorN:'Sy55'},
      {colorV:'#141416',colorN:'Sy90'},
      {colorV:'#FF8B00',colorN:'SYE27'},
      {colorV:'#D49F5D',colorN:'SyL28'},
      {colorV:'#827263',colorN:'SYQ28'},
    ]},
    {title:"内里文字",showtype:"text",placeN:'palm_text',activeI:'null',style:[{top:170,left:180,size:40,transform:0}],colorlist:[
      {colorV:'#5C4032',colorN:'CC28'},
      {colorV:'#772F09',colorN:'PA28'},
      {colorV:'#817F42',colorN:'PA42'},
      {colorV:'#D23745',colorN:'PT23'},
      {colorV:'#328A72',colorN:'PA47'},
      {colorV:'#874A97',colorN:'PT60'},
      {colorV:'#FEFEFE',colorN:'SY01'},
      {colorV:'#EBDFAF',colorN:'Sy03'},
      {colorV:'#A2A6A5',colorN:'Sy11'},
      {colorV:'#E7C84B',colorN:'Sy15'},
      {colorV:'#E3DA59',colorN:'Sy16'},
      {colorV:'#F47BA4',colorN:'SY19'},
      {colorV:'#F87936',colorN:'Sy22'},
      {colorV:'#F0C96A',colorN:'Sy27'},
      {colorV:'#5D8E78',colorN:'Sy40'},
      {colorV:'#91C771',colorN:'Sy43'},
      {colorV:'#2851BA',colorN:'Sy45'},
      {colorV:'#091023',colorN:'SY50'},
      {colorV:'#7BADC6',colorN:'Sy55'},
      {colorV:'#141416',colorN:'Sy90'},
      {colorV:'#FF8B00',colorN:'SYE27'},
      {colorV:'#D49F5D',colorN:'SyL28'},
      {colorV:'#827263',colorN:'SYQ28'},
    ]},
    {title:"字体",showtype:"radio",placeN:'text_font',activeI:'null',selectlist:[
			{showtxt:'Script',showvalue:'Script'},
			{showtxt:'Block',showvalue:'Block'},
    ]},
    {title:"特殊要求",showtype:"text",placeN:'palm_text',activeI:'null',style:[],colorlist:[]},
  ],
  // "colortxtdata":[
  // 	{colorV:'#884899',colorN:'紫色',activeI:'null',placeN:'customtext'},
  // 	{colorV:'#2178BA',colorN:'蓝色',activeI:'null',placeN:'customtext'},
  // 	{colorV:'#F7F911',colorN:'黄色',activeI:'null',placeN:'customtext'},
  // ]
}
